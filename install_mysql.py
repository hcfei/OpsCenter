#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Install MySQL 8 server on remote, initialize root password, create DB/user/table."""
import sys
import json
import time
import secrets
...
print("1. Installing mysql-server (this may take a few minutes)...")
code, out, err = run("dnf install -y mysql-server 2>&1 | tail -8")
print(out or err)
assert code == 0, "dnf install mysql-server failed"
...
TEMP_PASS = m.group(1)
print("   temp password found")

# 4. Change root password & create app user, database, table
print("4. Initializing root password, creating DB/user/table...")
SQL = (
...
code, out, err = run(
    f"mysql -uroot -p'{TEMP_PASS}' --connect-expired-password -e \"{SQL}\" 2>&1"
)
